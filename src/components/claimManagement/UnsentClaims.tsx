...
const UnsentClaims: React.FC = () => {
 ...
 const patientId = 2; // TODO: Replace with actual patientId from context/session
  ...
  const mockClaims = [
    ...
    { id: 28300, patientName: "Donald IV Pierson (8559)", type: "E-claim Primary", createdOn: "10/23/2025", payerName: "Horizon Young Grins", status: "readyForSubmission", hasAttachment: false, notes: "", description: "" },
  ...
  useEffect(() => {
    async function fetchClaims() {
      ...
      try {
        ...
        // Dynamic carriers
        const carriers = [...new Set(unsentClaims.map((c: any) => c.payerName).filter(Boolean))];
        setUniqueCarriers(carriers);
        // Mock fallback if empty
        if (unsentClaims.length === 0) {
  ...
  return (
    <div>
      <h2 className="font-semibold text-lg mb-2">Unsent Claims ({filteredClaims.length})</h2>
