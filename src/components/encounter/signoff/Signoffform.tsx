...
export default function Signoffform({
    ...
    useEffect(() => {
        ...
        if (encounterData.signoff && !value?.id) {
            const data = encounterData.signoff;
            setAttestationText(data.attestationText || "");
            setAckBilling(data.ackBilling ?? false);
            setLockEncounter(data.lockEncounter ?? true);
            setCosigner(data.cosigner || "");
            setNotes(data.notes || "");
        } else {
            ...
            setNotes(value?.comments || (value as any)?.notes || "");
        }
    }, [value, patientId, encounterId]);

    useEffect(() => {
        if (attestationText || notes || cosigner) {
            setEncounterSection(patientId, encounterId, "signoff", {
                attestationText,
                ackBilling,
                lockEncounter,
                cosigner,
    ...
    }, [attestationText, ackBilling, lockEncounter, cosigner, notes, patientId, encounterId]);
    ...
    async function saveDraft() {
        ...
        try {
            ...
            removeEncounterSection(patientId, encounterId, "signoff");
            onSaved(json.data!);
        } catch (e: unknown) {
